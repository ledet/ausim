...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xmi="http://www.omg.org/XMI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmi:version="2.0" xsi:schemaLocation="http://www.omg.org/XMI" xmlns:xalan="http://xml.apache.org/xalan" xmlns:simulinkmm="http://simulinkmm/1.0">
  ...
  <xsl:variable name="MatchedElementList" select="'|GraphicalInterface|ConfigManagerSettings|EditorSettings|SimulationSettings|Verification|ExternalMode|EngineSettings|ModelReferenceSettings|ConfigurationSet|ConcurrentExecutionSettings|SystemDefaults|BlockDefaults|AnnotationDefaults|LineDefaults|'"/><!-- Items being listed here that may need to be unskipped later SystemDefaults|BlockDefaults|AnnotationDefaults|LineDefaults -->
  ...
  <xsl:template match="*">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(contains($MatchedElementList, concat('|', $tag, '|')))"/> <!-- Skipped Elements -->
      <xsl:otherwise>
        <xsl:copy>
          <xsl:for-each select="@*">
            <xsl:attribute name="{name()}">
